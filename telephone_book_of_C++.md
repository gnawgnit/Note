...
本来是学校C++实践课程的题目，但是本人其实对C++并不怎么熟悉，因为先学的C语言，所以不是很纯的C++代码实现。直接上源码，注释都有。测试中文的话会有很多错误，一开始没想那么多。
...
int Tele_book::add_tele(){
	...
	strftime(new_tele->tele_message.tele_time, sizeof(new_tele->tele_message.tele_time),"%Y/%m/%d %X %A",localtime(&time_now));//时间格式
